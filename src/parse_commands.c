...
int control_socket, client_t* clients)
{
    ...
    if (!strcasecmp(command[0], "HELP")) {
        ...
        cwd_command(control_socket, command, clients);
        ...
        pass(server_data, control_socket, command, clients);
    } else if (!strcasecmp(command[0], "DELE")) {
        dele(control_socket, command);
    } /* else if (!strcasecmp(command[0], "PASV")) {
        pasv(server_data, control_socket, command, clients);
    }  */
